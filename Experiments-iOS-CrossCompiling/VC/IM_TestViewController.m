...
#ifndef USE_PNG
    imageView.image = [UIImage imageNamed:@"iphone.tif"];
#endif
...
- (void)posterizeImageWithNewMethod {
	...
	CGImageRef ref = createStandardImage(imageView.image.CGImage);
...
//    status = MagickForwardFourierTransformImage(magick_wand, MagickTrue); fftw
	if (status == MagickFalse) {
		ThrowWandException(magick_wand);
	}
...
- (void)posterizeImageWithoutCompression {
	...
	MagickWandGenesis();
	magick_wand = NewMagickWand();
	UIImage * src = imageView.image;
	CGImageRef srcCGImage = [src CGImage];
	NSData * srcData = (NSData *) CFBridgingRelease(CGDataProviderCopyData(CGImageGetDataProvider(srcCGImage)));
