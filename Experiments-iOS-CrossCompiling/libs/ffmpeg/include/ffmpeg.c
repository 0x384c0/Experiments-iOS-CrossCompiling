...
static void ffmpeg_cleanup(int ret)
{
    ...
    //===down==为了能够在ios中继续使用ffmpeg_main====需要清零========
    nb_filtergraphs=0;
    nb_output_files=0;
    nb_output_streams=0;
    nb_input_files=0;
    nb_input_streams=0;
    //=====up====为了能够在ios中继续使用ffmpeg_main====需要清零===
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            if(!strcmp(oc->oformat->name, "avi")) {
                ...
                } else if ((copy_tb<0 && av_q2d(dec_ctx->time_base)*dec_ctx->ticks_per_frame > 2*av_q2d(ist->st->time_base) && av_q2d(ist->st->time_base) < 1.0/500)
                    || copy_tb==0){
